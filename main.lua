...
local function show_spinner(duration, message)
	...
	for i = 1, iterations do
		...
		io.write(
			"\r" .. COLORS.CYAN .. spinner[idx] .. COLORS.RESET .. " " .. message
		)
		io.flush()
...
local function http_get(url)
	...
	local curl_cmd =
		string.format(
			"curl -sSL --connect-timeout 10 --max-time 30 %s -o %s",
			url,
			tmp_file
		)
...
local function init_project()
	print(
		COLORS.BOLD .. COLORS.BLUE .. "extify" .. COLORS.RESET .. " - Browser Extension Generator\n"
	)
	...
	if not json_content then
		color_print(
			COLORS.RED,
			"✖ Error: Could not connect to GitHub repository."
		)
		return
	end
	...
	for _, file_name in ipairs(selected.files) do
		...
		if content then
			...
			if write_file(file_name, processed) then
				...
				print(
					COLORS.RED .. "  ✖ " .. COLORS.RESET .. "Failed to write " .. file_name
				)
			end
		else
			print(
				COLORS.RED .. "  ✖ " .. COLORS.RESET .. "Failed to download " .. file_name
			)
		end
	end
...
local function show_help()
	print(
		COLORS.BOLD .. "extify" .. COLORS.RESET .. " - Browser Extension Boilerplate Generator"
	)
	print("\nUsage:")
	print(
		"  extify init      Initialize a new extension in the current directory"
	)
